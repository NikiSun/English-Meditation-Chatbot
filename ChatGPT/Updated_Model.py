...
os.environ["GOOGLE_APPLICATION_CREDENTIALS_STT"] = r"" # Insert the path to your Google Cloud credentials for Speech-to-Text (STT)
...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = r"" # Insert the path to your Google Cloud credentials for Text-to-Speech (TTS)
...
def generate_response(user_input, classification, practice_type):
    ...
    completion = client.chat.completions.create(
        model="" # Insert the path to your OpenAI model,
        messages=[
            {"role": "system", 
    ...
    # If the response is less than 200 words, request more content
    while len(ssml_response.split()) < 200:
        additional_completion = client.chat.completions.create(
            model="", # Insert the path to your OpenAI model,
            messages=[
                {"role": "user", 
                 "content": "Please elaborate further."},
                {"role": "assistant", 
                 "content": ssml_response}
            ]
        )
        ssml_response += " " + additional_completion.choices[0].message.content

    # Append SSML to file with two new lines after each response
    with open(r"", "a",  encoding="utf-8") as file: # Insert the path to the file where you want to save the responses
        file.write(ssml_response + "\n\n\n\n")
    
    return ssml_response

def text_to_speech_ssml(ssml_text):
    # Insert pauses after each comma, period, and question mark
